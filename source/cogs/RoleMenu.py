...
class RoleMenu(commands.Cog):
    ...
    async def create(self, msg, *args):
        ...
        for i in courses:
            ...
            currentMenu = {}
            for j in range(len(courses[i])):
                message += self.config.reactions[j] + " " + courses[i][j].upper() + "\n\n"
    ...
    async def edit(self, msg, *args):
        ...
        if args[1] == "add": # Add a new role to an existing role menu
            ...
            for i in range(len(self.config.reactions)):
                if self.config.reactions[i] not in self.config.rolemenuData[channelName][rolemenuKey]:
                    newReactionIndex = i
                    break
    ...
    async def on_raw_reaction_add(self, reaction):
        ...
        if channel.name in self.config.rolemenuData:
            ...
            if str(msg.id) in self.config.rolemenuData[channel.name] and msg.author == self.client.user: # The message id comes in as an integer but is serialised as a string when saved to JSON
                roleName = self.config.rolemenuData[channel.name][str(msg.id)][reaction.emoji.name]
